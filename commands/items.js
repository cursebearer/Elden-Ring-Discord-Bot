...
async function getItems() {
    ...
    const limit = 100;
    const pages = [0, 1, 2, 3, 4]; // Vamos buscar nas páginas de 0 a 4
...
async function items(client) {
    ...
    client.on('messageCreate', async (message) => {
        if (message.content.startsWith('!item ')) {
            const itemName = message.content.slice(6).toLowerCase(); // Remove o comando "!item " e converte para minúsculas
