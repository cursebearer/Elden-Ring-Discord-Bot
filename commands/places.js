...
async function getLocations() {
    ...
    const limit = 100;
    const pages = [0, 1]; // Vamos buscar nas páginas 0 e 1
...
function places(client) {
    client.on('messageCreate', async (message) => {
        if (message.content.startsWith('!place ')) {
            const searchTerm = message.content.slice(7).trim().toLowerCase(); // Remover o comando "!place " e converter para minúsculas
